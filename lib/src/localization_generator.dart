...
class LocalizationGenerator extends GeneratorForAnnotation<SheetLocalization> {
  ...
  static const _headers = {'Content-Type': 'text/csv; charset=utf-8', 'Accept': '*/*'};
  ...
  Future<String> _generateSource(Element2 element, ConstantReader annotation) async {
    ...
    void readCsv(File file) {
      ...
      classBuilder.writeln(csvParser.generateTranslationUsages(preservedKeywords, immediateTranslationEnabled));
    }
    ...
    if (docId.isNull || docId.stringValue.isEmpty) {
      ...
      final response = await http.get(Uri.parse(_urlFormat + docId.stringValue), headers: _headers);
      ...
      final formatString = LineSplitter().convert(generatedFile.readAsStringSync()).join(lineSeparator);
